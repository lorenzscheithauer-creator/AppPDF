...
class AppContainer(app: Application) { val settings = SettingsRepository(app); val history = HistoryRepository(HistoryDatabase.create(app).dao()); val pdfEngine = PdfBoxEngine(app); val limits = LimitPolicy(); val billing = BillingRepository(app, settings); val mergeAds = AdMobMergeAdService() }
